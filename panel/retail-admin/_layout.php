...
function retail_header(string $title, string $activeNav = ''): void
{
    ...
    echo '<meta name="csrf-token" content="' . htmlspecialchars(generateCSRFToken(), ENT_QUOTES) . '">';
...
function retail_footer(): void
{
    echo <<<'HTML'
<script>
window.kndRetailAdminAgent = async function(payload) {
  const meta = document.querySelector('meta[name="csrf-token"]');
  const token = meta ? meta.getAttribute('content') : '';
  const res = await fetch('api/agent_execute.php', {
    method: 'POST',
