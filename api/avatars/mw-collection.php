...
try {
    ...
    $catalog = mw_get_mw_avatars_catalog($pdo);
    ...
    foreach ($catalog as $item) {
        ...
        $modelPath = '/assets/avatars/models/' . $rarity . '/' . $slugUnderscore . '.glb';
        $thumbnailPath = (string) ($item['thumbnail_path'] ?? $item['asset_url'] ?? '');
        if ($thumbnailPath === '') {
            $thumbnailPath = '/assets/avatars/thumbs/' . $slugUnderscore . '.png';
        }
    ...
    error_log('avatars/mw-collection error: ' . $e->getMessage());
